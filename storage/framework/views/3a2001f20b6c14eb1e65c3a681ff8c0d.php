...
<?php echo $__env->make('templates.components.series-modal', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
...
<script>
    ...
    if (typeof window.editableElementsJson === 'undefined') {
        window.editableElementsJson = <?php echo json_encode($baseTemplate->editable_elements ?? [], 15, 512) ?>;
    }
    if (typeof window.baseTemplateId === 'undefined') {
        window.baseTemplateId = <?php echo e(isset($baseTemplate) && $baseTemplate ? $baseTemplate->id : 'null'); ?>;
        console.log('Устанавливаем baseTemplateId:', window.baseTemplateId);
    }
    if (typeof window.removeUrl === 'undefined') {
        window.removeUrl = '<?php echo e(route("templates.remove-cover")); ?>';
    }
    if (typeof window.previewUrl === 'undefined') {
        window.previewUrl = '<?php echo e(route('templates.preview', $baseTemplate->id ?? 0)); ?>';
    }
    if (typeof window.storeUrl === 'undefined') {
        window.storeUrl = '<?php echo e(route("user-templates.store")); ?>';
    }
    if (typeof window.myTemplatesUrl === 'undefined') {
        window.myTemplatesUrl = '<?php echo e(route("my-templates")); ?>';
    }
...
<?php /**PATH C:\OSPanel\domains\tyty\resources\views/templates/components/template-scripts.blade.php ENDPATH**/ ?>